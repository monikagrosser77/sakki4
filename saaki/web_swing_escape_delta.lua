...
if TargetParent:FindFirstChild("SakiScriptsWebSwingUI") then
    TargetParent:FindFirstChild("SakiScriptsWebSwingUI"):Destroy()
end
...
local GAME_NAME = "+1 WEB SWING ESCAPE"
...
ScreenGui.Name = "SakiScriptsWebSwingUI"
            ...
            while AutoTrainState do
                pcall(function()
                    ...
                    if char then
                        ...
                        -- Remote events triggering for Training / Speed / Swing
                        for _, obj in pairs(ReplicatedStorage:GetDescendants()) do
                            if not AutoTrainState then break end
                            if obj:IsA("RemoteEvent") then
                                local lowerName = obj.Name:lower()
                                if lowerName:find("train") or lowerName:find("speed") or lowerName:find("power") or lowerName:find("swing") or lowerName:find("click") then
                                    obj:FireServer()
                                    obj:FireServer(1)
                                end
                            end
...
-- 2. AUTO WIN (INSTANT WIN / GOAL TELEPORT)
            ...
            while AutoWinState do
                pcall(function()
                    ...
                    for _, obj in pairs(ReplicatedStorage:GetDescendants()) do
                        ...
                        if obj:IsA("RemoteEvent") then
                            local lowerName = obj.Name:lower()
                            if lowerName:find("win") or lowerName:find("escape") or lowerName:find("finish") or lowerName:find("reward") or lowerName:find("reach") then
                                obj:FireServer()
                                obj:FireServer(1)
                            end
                        end
                    ...
                    -- 2. Teleport to Win / Goal / Finish pads in Workspace
                    if root then
                        for _, part in pairs(Workspace:GetDescendants()) do
                            if not AutoWinState then break end
                            if part:IsA("BasePart") then
                                local lowerName = part.Name:lower()
                                if lowerName:find("win") or lowerName:find("finish") or lowerName:find("goal") or lowerName:find("end") or lowerName:find("escape") then
                                    -- Touch simulation
...
print("Saki Scripts UI (+1 Web Swing Escape) Loaded Successfully!")
